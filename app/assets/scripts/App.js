...
$('.slider-event__slider').each(function() {
  var $this = $(this);
  var $group = $this.find('.slider-event__slide_group');
  var $slides = $this.find('.slider-event__slide');
  var bulletArray = [];
  var currentIndex = 0;
  var timeout;
  ...
  $('.slider-event__next_btn').on('click', function() {
    if (currentIndex < ($slides.length - 1)) {
      move(currentIndex + 1);
    } else {
      move(0);
  ...
  $('.slider-event__previous_btn').on('click', function() {
    if (currentIndex !== 0) {
      move(currentIndex - 1);
    } else {
      move(3);
  ...
  $.each($slides, function(index) {
    var $button = $('<a class="slider-event__slide_btn">&bull;</a>');
    ...
    $button.on('click', function() {
      move(index);
    }).appendTo('.slider-event__slide_buttons');
    bulletArray.push($button);
  });
